...
async function attack() {
  ...
  console.log(
    chalk(`${name} ATTACKED!! Enemy health is now ${initailHealthEnemy}.`)
  );
  console.log(
    chalk(
      `Your health is now ${initailHealthPlayer} due to your own attacks effect.`
    )
  );
  ...
  if (initailHealthEnemy <= 0) {
    ...
    console.log(
      chalk.redBright(`You died because of the effect of your own attack!`)
    );
    gameEnd = true;
  }
}
...
async function defend() {
  ...
  console.log(
    chalk(`ENEMY ATTACKED!! Your health is now ${initailHealthPlayer}.`)
  );
  if (initailHealthEnemy <= 0) {
    console.log(
      chalk.redBright(
        `Great! Your enemy died because of its own attack effect!`
      )
    );
    gameEnd = true;
  } else if (initailHealthPlayer <= 0) {
...
async function playGame() {
  while (true) {
    ...
    if (ans.name.trim() == "") {
      ...
      name = ans.name;
      console.log(
        chalk.greenBright(`Your hero name is ${name}! Be prepared..`)
      );
      await startGame();
      initailHealthPlayer = 100;
