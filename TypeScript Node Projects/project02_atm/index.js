...
async function withdraw() {
    ...
    if (amount.money <= balance) {
        ...
        console.log(chalk.greenBright(`Withdrawing ${amount.money} from account. Your new balance is ${balance}.`));
    }
    else {
        console.log(chalk.redBright(`Insufficient balance in account to withdraw. Your current balance is ${balance}.`));
    }
}
function checkAccount(accountNum) {
...
async function transfer() {
    ...
    if (id != -1) {
        ...
        if (amount.money <= balance) {
            balance = balance - amount.money;
            console.log(chalk.greenBright(`Tranferring ${amount.money} from your account to to ${accounts[id].name}. Your new balance is ${balance}.`));
        }
        else {
            console.log(chalk.redBright(`Insufficient balance in account to transfer. Your current balance is ${balance}.`));
        }
    }
    else {
...
async function login() {
    ...
    while (tries > 0) {
        ...
        if (credential.accNum == accNum && credential.accPin == pin) {
            ...
            if (tries > 1) {
                console.log(chalk.redBright("Invalid account number or pin! Try again.."));
            }
            failed = true;
        }
        tries--;
    ...
    if (failed) {
        console.log(chalk.redBright("You failed to enter correct account number and pin for 3 times.\nSystem closing. Bye!"));
    }
}
async function main() {
    console.clear();
